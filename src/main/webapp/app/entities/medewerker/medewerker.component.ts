...
export class MedewerkerComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.medewerkerService
      ...
      .subscribe((res: HttpResponse<IMedewerker[]>) => this.paginateMedewerkers(res.body, res.headers));
  }
  ...
  trackId(index: number, item: IMedewerker) {
    return item.id;
  }

  registerChangeInMedewerkers() {
  ...
  protected paginateMedewerkers(data: IMedewerker[], headers: HttpHeaders) {
